...
public class PlayerMovement : MonoBehaviour
{
    ...
    private void ProcessOtherInputs() 
    {
		...
		if (canDodge && Input.GetKeyDown(KeyCode.P))
		{
			...
			if (!isGrounded)
			{
				...
				transform.position += movement * (dodgeSpeed / 1.5f) * Time.deltaTime * horizontalSpeed;
			}
			else
			{ 
			    Vector3 movement = new Vector3(Input.GetAxis("Horizontal"), 0f, 0f);

			    transform.position += movement * dodgeSpeed * Time.deltaTime * horizontalSpeed;
		    }
			Invoke("SecondDodge", .01f);

			Invoke("SecondDodge", .01f);

			canDodge = false;

            Invoke("AllowDodge", dodgeCooldown);
		}
